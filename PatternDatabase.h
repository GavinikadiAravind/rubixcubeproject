...
public:
    ...
    virtual uint32_t getDatabaseIndex(const Generalrubikscube &cube) const = 0;

    virtual bool setNumMoves(const Generalrubikscube &cube, const uint8_t numMoves);

    virtual bool setNumMoves(const uint32_t ind, const uint8_t numMoves);

    virtual uint8_t getNumMoves(const Generalrubikscube &cube) const;
